...
| **Tabs** | Hover lightens whichever tab the pointer is over (the active one too), resolved against last frame's positions so it actually fires; active-tab text stays high-contrast on the fill (never accent-on-blue); the accent **indicator line = selection, always on for the active tab, on the top (outer) edge away from the content**; focus thickens the line on a vector strip. A character grid has no room for the line, so the active tab wears the selection **fill** under the §4b ordering — the loud `selection_active_bg` only while the strip is focused, the muted `selection_inactive_bg` when it is not (the same pair ListView rows use), so an unfocused strip never shows the loud accent. | ✅ done |
