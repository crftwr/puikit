...
| `TextEdit` | `text_edit.py` | Single-line editable text; full IME/composition, mouse + keyboard selection, word-wise caret move/delete (Ctrl / Alt-Option), Cmd-Left/Right to the line ends and Cmd-Backspace deleting back to the start, clipboard copy/cut/paste. Scrolls horizontally keeping a few columns of text visible past the caret on either side, and pulls hidden text back in when an edit shortens the line. `mask="•"` makes it a password field (glyphs shown masked; copy/cut disabled) |
...
| `ListView` | `list.py` | Scrollable selectable rows; text by default (over-long rows clip, or elide with `ellipsis`/`elide_where`), or a widget per row via `row_factory`; `allow_no_selection` permits `selected == -1`, drawn with no highlight at all |
